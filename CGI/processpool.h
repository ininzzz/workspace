...
processpool<T>::processpool(int listenfd,int process_number):
    ...
    for(int i=0;i<process_number;i++){
        ...
        // 子进程用m_pipefd[1]读写数据
        // 注意这里子进程中除了m_sub_process[m_idx]中的数据，其他都是用不到的
        // 只有父进程的m_sub_process才存有完整的子进程的信息
        else{
...
void processpool<T>::setup_sig_pipe(){
    ...
    // 两端都非阻塞(addfd调用里也有setnonblocking)
    setnonblocking(sig_pipefd[1]);
    addfd(m_epollfd,sig_pipefd[0]);

    // 当子进程终止时，向父进程发送sigchld信号
...
void processpool<T>::run_child(){
    ...
    int num=0,ret=-1;
    while(!m_stop){
        num=epoll_wait(m_epollfd,events,MAX_EVENT_NUMBER,-1);
    ...
    while(!m_stop){
        ...
        for(int i=0;i<number;i++){
            ...
            if(sockfd==m_listenfd){
                ...
                printf("send requset to child %d\n",i);
            }
            // 下面处理父进程接收到的信号
            else if((sockfd==sig_pipefd[0])&&(events[i].events&EPOLLIN)){
                int sig;
